...
export const Compra = async (investimento: Omit<InvestirDto, 'id'>) => {
    if (investimento.investimentoId <= 3) {
        ...
        return await InvestimentoFixoProvider.compra(investimento)
        ...
        if (!investimento.quantidadeCotas || !investimento.valorCota) {
         ...
         return 'compra de transação variavel não implementado'//todo Service de investimento variavel
    }
}